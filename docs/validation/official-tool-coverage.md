...
| 半角空白・半角括弧 | Checker tools | Review finding in Japanese context | Requires source-book confirmation |
| OCR confusables | 校正ツール and manual searches | Review finding in suspicious script context | Requires source-book confirmation |
| `ケ` / `ヶ` and ruby grouping | Manual comparison | Review findings | Never silently corrected |
| Opening legend and closing bibliography | Official checklist | Review findings | Structure can be detected; content remains editorial |
| Base-text, semantic layout, rights | Proofreading manual | Manual catalog and TUI items | Never counted as automatically confirmed |
| Difference/history reports | 相違点・修正履歴 tools | Session-only staged diff | Persistent baseline/history is outside scope |
...
The executable mapping is generated by `aozora-proof rules --format json` and
enforced by the catalog coverage tests. See
[External oracle comparison](external-oracle-comparison.md) for the behavioral
boundary and [Public corpus baseline](public-corpus-baseline.md) for measured
results.
